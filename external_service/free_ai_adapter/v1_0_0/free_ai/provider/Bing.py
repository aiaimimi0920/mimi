...
from ..self_typing       import AsyncResult, Messages, AsyncGenerator,Union
    ...
    async def create_async_generator(
        ...
        **kwargs
        ...
        if proxies:
            ...
            if "https" in proxies:
                proxy = "http://"+proxies["https"]
        if stream:
            async for data in stream_generate(prompt, tone, image, context, proxy, cookies):
                yield data
        else:
            all_data = ""
            async for data in stream_generate(prompt, tone, image, context, proxy, cookies):
