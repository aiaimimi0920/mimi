...
func _on_init()->void:
	super._on_init()
	set_plugin_info(plugin_name,"update_bot","mimi",plugin_version,"update plugin content","plugin",
		{"file_list":["v1_0_1"]})
...
func download_load_plugin_pck(cur_plugin_name:String="",min_version:int=0,max_version:int=1000000000):
	...
	if not use_version:
		...
		if download_method == "ipfs":
			...
			path = await file_list.fs_download({"path":target_plugin_info["store_info"]["ipfs_id"],"save_dir":""},"ipfs_adapter")
			...
			path = await file_list.fs_download({"path":target_plugin_info["store_info"]["sub_path"],
				"save_dir":"","region":target_plugin_info["store_info"]["region"],"bucket":target_plugin_info["store_info"]["bucket"],
				"access_key":"","secret_key":"","service":target_plugin_info["store_info"]["service"]},"aws_adapter")
			...
			path = await file_list.fs_download({"path":target_plugin_info["store_info"]["sub_path"],
				"save_dir":"","region":target_plugin_info["store_info"]["region"],"bucket":target_plugin_info["store_info"]["bucket"],
				"access_key":target_plugin_info["store_info"]["read_access_key"],"secret_key":target_plugin_info["store_info"]["read_secret_access_key"],
				"service":target_plugin_info["store_info"]["service"],"account_id":target_plugin_info["store_info"]["account_id"]},"cloudflare_adapter")
