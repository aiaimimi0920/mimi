	...
	if platform_name in ["ipfs","cloudflare","aws"]:
		...
		if platform_name == "ipfs":
			...
			#ipfs_id = await file_list.fs_upload_form(root_path,"",0,"",
				#"","","","","",pinata_key)
			var cur_fs_upload_form_data = {
				"path":root_path,
				"pinata_key":pinata_key,
			}
			ipfs_id = await file_list.fs_upload_form(cur_fs_upload_form_data,"ipfs_adapter")
			...
			var fs_upload_form_result = await file_list.fs_upload_form(cur_fs_upload_form_data,"cloudflare_adapter")
			...
			var fs_upload_form_result = await file_list.fs_upload_form(cur_fs_upload_form_data,"aws_adapter")
			sub_path = fs_upload_form_result[0]
