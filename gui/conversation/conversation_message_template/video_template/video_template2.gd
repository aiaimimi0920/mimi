...
func set_stream_path(stream_path):
	%FFmpegMediaPlayer.load_path_async(stream_path)
	
	var target_size = Vector2(message.video_width, message.video_height)
	if target_size:
		%VideoTextureRect.expand = true
		%VideoTextureRect.custom_minimum_size = target_size
	else:
		%VideoTextureRect.expand = false
