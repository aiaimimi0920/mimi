...
func presigned_URL(
	...
	if account_id == "" or account_id==null:
		account_id = plugin_node.service_config_manager.account_id


	var host = account_id + "." + service  + ".cloudflarestorage.com"
	...
	var credential_scope = date_stamp + "/" + region + "/" + "s3" + "/" + "aws4_request"
	var string_to_sign = (
		algorithm
		+ "\n"
		+ amz_date
...
func http_request(
	...
	if bucket == "" or bucket==null:
		bucket = plugin_node.service_config_manager.bucket_name
	query_url_path = "/"+bucket+query_url_path
	...
	var connect_host_url = "https://"+host

	var request_query_url_path = query_url_path
	if query_string_str != "":
		request_query_url_path += "?" + query_string_str

	if method==HTTPClient.METHOD_GET and use_native==false:
		if cmd:
			cmd.time += 1
		
		var downloader_list = await PluginManager.get_plugin_instance_by_script_name("downloader_list")
		var options = {}
		options["out"] = save_path.get_file()
		options["dir"] = save_path.get_base_dir()
		options["header"] = "\n".join(headers)
		var download_gid = await downloader_list.new_task([connect_host_url+query_url_path],
		...
		var err = http.connect_to_host(connect_host_url)
		assert(err == OK)
