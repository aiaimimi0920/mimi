...
func fs_list(sub_path, region="", bucket="", access_key="", secret_key="", service="",account_id="", all_files = null,all_dirs = null,continuation_token=null):
	...
	await service_utils.http_request(
		...
		"",
		null,
		fs_list_all_chunk_call.bind(all_dirs,all_files,[sub_path, region, bucket, access_key, secret_key, service,account_id, continuation_token]),
		null,
		region,
		bucket,
		access_key,
...
func fs_list_all_chunk_call(rb, all_dirs, all_files, call_cmd):
	...
	if all_data.get("IsTruncated",false) == true:
		await fs_list(call_cmd[0], call_cmd[1], call_cmd[2], call_cmd[3], call_cmd[4], call_cmd[5],call_cmd[6], all_files, all_dirs, all_data["NextContinuationToken"])
...
func fs_upload_form(sub_path, base_dir_path="", region="", bucket="", access_key="", secret_key="", service="",
	...
	var result_account_id = account_id if (account_id and account_id != "") else plugin_node.service_config_manager.account_id
	
	return [result_path.trim_prefix("/"), result_region, result_bucket, result_service, result_account_id]
...
func fs_download(sub_path, cur_save_dir="", region="", bucket="", access_key="", secret_key="", service="",
	...
	var call_md5 = (sub_path+cur_save_dir+region+bucket+access_key+secret_key+service+account_id).md5_text()
	var cmd
	if call_md5 in now_download_call:
		...
		var key_md5 = (bucket+account_id).md5_text()
		if access_key !="" and secret_key!="":
			key_map[key_md5] = [access_key, secret_key]
			
		## Explain the key logic. If access_key and secret_key are empty strings, it is assumed to be accessing one's own library.
		## If the default access_key and secret_key are empty strings, it will downgrade to access key for access.
		## That is, if you download public files from others, you need another key. The key is not managed here, but managed when it is passed in.
		## The reason for being different from AWS is that AWS supports anonymous access, while Cloudflare only supports access with a key.
		if access_key =="" or secret_key=="":
			if key_md5 in key_map:
				access_key = key_map[key_md5][0]
				secret_key = key_map[key_md5][1]
		...
		if access_key =="" or secret_key=="":
			access_key = plugin_node.service_config_manager.read_access_key
			secret_key = plugin_node.service_config_manager.read_secret_access_key
			
		var all_files_dirs = await fs_list(sub_path, region, bucket, access_key, secret_key, service, account_id)
		...
		for sub_file_path in all_files:
			...
			## GitHub Copilot: Note that this place is a coroutine, but don't wait, because I want to download multiple files in parallel, so you can wait in the subsequent request_finished.
			service_utils.http_request(
				HTTPClient.METHOD_GET,
				sub_file_path,
				"",
