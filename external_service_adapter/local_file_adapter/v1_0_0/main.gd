...
func _on_init()->void:
	super._on_init()
	set_plugin_info(plugin_name,"local_file_adapter","mimi",plugin_version,"It is possible to call the functions of local file",
		"service",{"file_list":["v1_0_1"]})
...
func _ready()->void:
	service_local = load(get_absolute_path("modules/local.gd")).new()
...
func start()->void:
	...
	service_local.name = "local"
	add_child(service_config_manager,true)
	add_child(service_local,true)
...
func fs_list(adapter_need_data):
	...
	return await service_local.fs_list(path)
...
func get_fs_upload_form_need_args():
	return {"path":"","base_dir_path":"","target_path":""}

## Form upload file
func fs_upload_form(adapter_need_data):
	var path = adapter_need_data["path"]
	var base_dir_path = adapter_need_data["base_dir_path"]
	var target_path = adapter_need_data["target_path"]
	return await service_local.fs_upload_form(path, base_dir_path, target_path)
...
func fs_download(adapter_need_data):
	...
	return await service_local.fs_download(path, save_dir)
