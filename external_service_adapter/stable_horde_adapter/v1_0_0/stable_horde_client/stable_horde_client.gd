...
const R2ImageRetriever = preload("./r2_image_retriever.gd")
...
const AIImageTexture = preload("./AIImageTexture.gd")
...
#	print_debug(submit_dict)
	...
	var headers = [
		...
		"apikey: " + api_key,
		"Client-Agent: " + "Lucid Creations:" + "v2.1.0" + ":db0#1625",
	]
	var error = request(plugin_node.service_config_manager.aihorde_url + "/api/v2/generate/async", headers, HTTPClient.METHOD_POST, body)
	if error != OK:
...
func check_request_process(operation := OngoingRequestOperations.CHECK) -> void:
	...
	var url = plugin_node.service_config_manager.aihorde_url + "/api/v2/generate/check/" + async_request_id
	var method = HTTPClient.METHOD_GET
	if operation == OngoingRequestOperations.GET:
		url = plugin_node.service_config_manager.aihorde_url + "/api/v2/generate/status/" + async_request_id
	...
	var error = request(
		url, 
		["Client-Agent: " + "Lucid Creations:" + str(plugin_version) + ":db0#1625"], 
		method)
...
func _extract_images(generations_array: Array) -> void:
	...
	for img_dict in generations_array:
		...
		if 'https' in img_dict["img"]:
			...
			image_retriever.connect(
					"retrieval_success", 
					Callable(self,"_on_r2_retrieval_success").bindv([img_dict,  timestamp, generations_array.size()]))
