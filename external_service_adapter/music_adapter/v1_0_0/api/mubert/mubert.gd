...
func create_music(tags, duration):
	...
	var result = await C_S_CREATE_MUSIC(-1, data)
	return result


## 方法名与子协议同名
func C_S_CREATE_MUSIC(server_syncId, content):
	## C_S_TEST为当前调用的方法的字符串，暂时没有找到可以直接获得当前方法名字的方法
	return await plugin_node.send_service_request(server_syncId, content, Callable(self, "C_S_CREATE_MUSIC"))
	

## 方法名与子协议同名
func S_C_CREATE_MUSIC(server_syncId, content):
	pass
