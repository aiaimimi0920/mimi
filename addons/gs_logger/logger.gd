...
#	PUBLIC
func add_file_appender_by_name_path(log_path:String, log_name:String=""):
	var appender = FileAppender.new(log_path)
	appender.logger_name = log_name
...
func info(message : String, category : String = CATEGORY_GENERAL, sub_log_name:=""):
	_append(LOG_LEVEL_INFO, message, category, sub_log_name)


#Function: fine
#	Log a Message at a Fine level.
func fine(message : String, category : String = CATEGORY_GENERAL, sub_log_name:=""):
	_append(LOG_LEVEL_FINE, message, category, sub_log_name)


#Function: trace
#	Log a Message at a Trace level.
func trace(message : String, category : String = CATEGORY_GENERAL, sub_log_name:=""):
	_append(LOG_LEVEL_TRACE, message, category, sub_log_name)


#Function: debug
#	Log a Message at a Trace level.
func debug(message : String, category : String = CATEGORY_GENERAL, sub_log_name:=""):
	_append(LOG_LEVEL_DEBUG, message, category, sub_log_name)


#Function: warn
#	Log a Warning Message.
func warn(message : String, category : String = CATEGORY_GENERAL, sub_log_name:=""):
	_append(LOG_LEVEL_WARN, message, category, sub_log_name)


#Function: error
#	Log an Error Message.
func error(message : String, category : String = CATEGORY_GENERAL, sub_log_name:=""):
	_append(LOG_LEVEL_ERROR, message, category, sub_log_name) 


#Function: fatal
#	Log an Error Message.
func fatal(message : String, category : String = CATEGORY_GENERAL, sub_log_name:=""):
	_append(LOG_LEVEL_FATAL, message, category, sub_log_name)
...
func _append(level, message = "", category = CATEGORY_GENERAL, sub_log_name:=""):
	...
	for appender in logger_appenders:
		if level <= appender.logger_level:
			appender.append(LoggerMessage.new(level, message, category, logger_line))
	
	## 如果有子log，那么在子log中也打印输出
	if sub_log_name!="":
		for appender in logger_appenders:
			if level <= appender.logger_level and sub_log_name==appender.logger_name:
				appender.append(LoggerMessage.new(level, message, category, logger_line))
